...
class PiwikTrackingListener
{
    ...
    private $documentFields = ['id, product, docstrct', 'log_type'];
     ...
     */
    public function __construct(ReportService $reportService, Container $container)
    {
    ...
    public function onKernelResponse(FilterResponseEvent $event)
    {
        ...
        if ($isResponseSuccessful) {
            $piwiktracker_url = $this->container->getParameter('piwiktracker_url');

            if (!empty($piwiktracker_url)) {
                ...
                if (!empty($action) && ($action === '_detail' || $action === '_search' || $action === '_search_advanced' || $action === '_download_pdf')) {
                    ...
                    if ($piwikPing) {
                        $userIdentifier = $this->reportService->getUserIdentifier();
                        ...
                        if (!empty($userIdentifier)) {
                            ...
                            if ($action === '_detail' || $action === '_download_pdf') {
                                ...
                                if ($action === '_detail') {
                                    ...
                                    $document = $this->reportService->getDocument($idSearchStr, $documentId, $this->documentFields);
                                    ...
                                    $document = $this->reportService->getDocument($workSearchStr, $workId, $this->documentFields);
                                ...
                                if ((isset($documentId) && !empty($documentId)) &&
                                    ...
                                    $configMonographDocumentType = $this->container->getParameter('doc_type_monograph');
                                    $configPeriodicalDocumentType = $this->container->getParameter('doc_type_periodical');
                                    ...
                                    if ($page === null) {
                                        ...
                                        if (isset($documentType) && $documentType === $configMonographDocumentType && !isset($activeChapterId)) {

                                            // Book Report 1 Tracking Identifier (br1 = Book Report 1)
                                            $br1TrackingIdentifier = sprintf('%s:%s:%s', $userIdentifier, $product, $documentId);

                                            if ($searchFlag) {
                                                // Adds Search Identifier to Book Report 1 Tracking Identifier
                                                $br1TrackingIdentifier = sprintf($br1TrackingIdentifier.':%s', $searchFlag);
                                            }

                                            if ($isThisAPdfDownload) {
                                                // Adds pdf-Document Identifier to Book Report 1 Tracking Identifier
                                                $br1TrackingIdentifier = sprintf($br1TrackingIdentifier.':%s', $pdfDocumentType);
                                            }

                                            // Tracks Clicks for Book Report 1 (br1 = Book Report 1)
                                            if (!empty($br1TrackingIdentifier)) {
                                                $br1CVAR = sprintf('cvar={"%d":["%s","%s"]}', self::BOOK_REPORT_1_SLOT, self::BOOK_REPORT_1, $br1TrackingIdentifier);
                                                $br1TrackingRequest = sprintf('?%s&%s&%s', $br1CVAR, $idsiteStr, $recStr);
                                                $br1Promise = $client->postAsync($br1TrackingRequest);
                                                $br1Promise->wait();
                                            }
                                        } elseif (isset($documentType) && $documentType === $configPeriodicalDocumentType) {

                                            // Journal Report 1 Tracking Identifier
                                            $jr1TrackingIdentifier = sprintf('%s:%s:%s', $userIdentifier, $product, $documentId);

                                            if ($isThisAPdfDownload) {
                                                // Adds pdf-Document Identifier to Journal Report 1 Tracking Identifier
                                                $jr1TrackingIdentifier = sprintf($jr1TrackingIdentifier.':%s', $pdfDocumentType);
                                            }

                                            // Tracks Journal HTML and PDF Clicks for Journal Report 1 (jr1)
                                            if (!empty($jr1TrackingIdentifier)) {
                                                $jr1CVAR = sprintf('cvar={"%d":["%s","%s"]}', self::JOURNAL_REPORT_1_SLOT, self::JOURNAL_REPORT_1, $jr1TrackingIdentifier);
                                                $jr1TrackingRequest = sprintf('?%s&%s&%s', $jr1CVAR, $idsiteStr, $recStr);
                                                $jr1Promise = $client->postAsync($jr1TrackingRequest);
                                                $jr1Promise->wait();
                                            }
                                        }
                                        ...
                                        if (!isset($activeChapterId)) {
                                            ...
                                            $dr1RVTrackingIdentifier = sprintf('%s:%s:%s', 'RV', $userIdentifier, $product);
                                            ...
                                            if ($searchFlag) {
                                                ...
                                                $dr1RCTrackingIdentifier = sprintf('%s:%s:%s', 'RC', $userIdentifier, $product);
                                            ...
                                            if (!empty($dr1RVTrackingIdentifier)) {
                                                $dr1RVCVAR = sprintf('cvar={"%d":["%s","%s"]}', self::DATABASE_REPORT_1_SLOT, self::DATABASE_REPORT_1, $dr1RVTrackingIdentifier);
                                                $dr1RVTrackingRequest = sprintf('?%s&%s&%s', $dr1RVCVAR, $idsiteStr, $recStr);
                                                $dr1RVPromise = $client->postAsync($dr1RVTrackingRequest);
                                                $dr1RVPromise->wait();
                                            }
                                            ...
                                            if (isset($dr1RCTrackingIdentifier) && !empty($dr1RCTrackingIdentifier)) {
                                                $dr1RCCVAR = sprintf('cvar={"%d":["%s","%s"]}', self::DATABASE_REPORT_1_SLOT, self::DATABASE_REPORT_1, $dr1RCTrackingIdentifier);
                                                $dr1RCTrackingRequest = sprintf('?%s&%s&%s', $dr1RCCVAR, $idsiteStr, $recStr);
                                                $dr1RCPromise = $client->postAsync($dr1RCTrackingRequest);
                                                $dr1RCPromise->wait();
                                            }

                                            // Tracks Record Views for Platform Report 1 (pr1 = Platform Report 1, RV = Record Views)
                                            if (!empty($pr1RVTrackingIdentifier)) {
                                                $pr1RVcvar = sprintf('cvar={"%d":["%s","%s"]}', self::PLATFORM_REPORT_1_SLOT, self::PLATFORM_REPORT_1, $pr1RVTrackingIdentifier);
                                                $pr1RVTrackingRequest = sprintf('?%s&%s&%s', $pr1RVcvar, $idsiteStr, $recStr);
                                                $pr1RVPromise = $client->postAsync($pr1RVTrackingRequest);
                                                $pr1RVPromise->wait();
                                            }
                                            ...
                                            if (isset($pr1RCTrackingIdentifier) && !empty($pr1RCTrackingIdentifier)) {
                                                $pr1RCCVAR = sprintf('cvar={"%d":["%s","%s"]}', self::PLATFORM_REPORT_1_SLOT, self::PLATFORM_REPORT_1, $pr1RCTrackingIdentifier);
                                                $pr1RCTrackingRequest = sprintf('?%s&%s&%s', $pr1RCCVAR, $idsiteStr, $recStr);
                                                $pr1RCTPromise = $client->postAsync($pr1RCTrackingRequest);
                                                $pr1RCTPromise->wait();
                                            }
                                        }
                                    }

                                    if (isset($activeChapterId) && !empty($activeChapterId) && isset($documentType) && $documentType === $configMonographDocumentType) {

                                        // Book Report 2 Tracking Identifier (br2 = Book Report 2)
                                        $br2TrackingIdentifier = sprintf('%s:%s:%s:%s', $userIdentifier, $product, $documentId, $activeChapterId);

                                        if ($isThisAPdfDownload) {
                                            // Adds pdf-Document Identifier to Journal Report 2 Tracking Identifier
                                            $br2TrackingIdentifier = sprintf($br2TrackingIdentifier.':%s', $pdfDocumentType);
                                        }

                                        // Tracks Book Chapter Clicks for Book Report 2 (br2 = Book Report 2)
                                        if (!empty($br2TrackingIdentifier)) {
                                            $br2CVAR = sprintf('cvar={"%d":["%s","%s"]}', self::BOOK_REPORT_2_SLOT, self::BOOK_REPORT_2, $br2TrackingIdentifier);
                                            $br2TrackingRequest = sprintf('?%s&%s&%s', $br2CVAR, $idsiteStr, $recStr);
                                            $br2Promise = $client->postAsync($br2TrackingRequest);
                                            $br2Promise->wait();
                                        }
                                    }
                                }
                                ...
                                if (!empty($collection) && $collection !== 'all') {
                                    ...
                                    $dr1SearchTrackingIdentifier = sprintf('%s:%s:%s', 'RS', $userIdentifier, $collection);

                                    // Tracks Regular Searches for Database Report 1 (dr1 = Database Report 1)
                                    $dr1SearchCVAR = sprintf('cvar={"%d":["%s","%s"]}', self::DATABASE_REPORT_1_SLOT, self::DATABASE_REPORT_1, $dr1SearchTrackingIdentifier);
                                    $dr1SearchTrackingRequest = sprintf('?%s&%s&%s', $dr1SearchCVAR, $idsiteStr, $recStr);
                                    $dr1SearchPromise = $client->postAsync($dr1SearchTrackingRequest);
                                ...
                                $pr1SearchCVAR = sprintf('cvar={"%d":["%s","%s"]}', self::PLATFORM_REPORT_1_SLOT, self::PLATFORM_REPORT_1, $pr1SearchTrackingIdentifier);
                                $pr1SearchTrackingRequest = sprintf('?%s&%s&%s', $pr1SearchCVAR, $idsiteStr, $recStr);
                                $pr1SearchPromise = $client->postAsync($pr1SearchTrackingRequest);
